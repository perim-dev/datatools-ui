...
const receiveTripsFromText = createAction(
  'RECEIVE_TRIPS_FROM_TEXT',
  (payload: { pattern: Pattern, trips: Array<Trip> }) => payload
)
...
export type EditorTripActions = ActionType<typeof addNewTrip> |
  ...
  ActionType<typeof receiveTripsFromText> |
...
export function updateTripFromText(csvTrip, propsRoute) {

  return function (dispatch: dispatchFn, getState: getStateFn) {
    const arrayTrip = csvTrip.split('\n').map(i => {
      const trimedString = trim(i);
    ...
    console.log('função para receber, parsear e guardar as trips', {attrFromHeader, dataTrip, ObjParsedTrip})
    dispatch(receiveTripsFromText({ trips: ObjParsedTrip, pattern: [{ ok: true }], propsRoute }))
  }
}
